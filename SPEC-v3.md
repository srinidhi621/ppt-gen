...
The scanner checks above are the **automated subset** of the full visual hygiene suite (§10.2). The complete suite of 26 checks is defined in `assets/benchmarks/v3_visual_hygiene_checks.xlsx`; checks marked `scanner_automatable = Yes` are implemented here, while the remainder are verified during multimodal review (Phase 8).
...
26 test prompts in `assets/benchmarks/v3_test_prompts.xlsx` (generated by `scripts/generate_benchmark_xlsx.py`). Organized in 7 sections:

| Section | Tests | What it exercises |
|---|---|---|
| Core archetypes | TP-01 – TP-10 | One prompt per active archetype, mid-density, standard audience |
| Untested archetypes | TP-11 – TP-13 | `quote_callout`, `section_break`, `stat_list_with_icons` |
| Edge cases | TP-14 – TP-16 | Sparse content, capacity overflow (feasibility gate), ambiguous archetype |
| Audience variations | TP-17 – TP-18 | Board-level (ultra-sparse) vs. technical team (data-rich) |
| Content type variations | TP-19 – TP-20 | Narrative case study, sales-oriented persuasion |
| Deck-level tests | TP-21 – TP-24 | Multi-slide decks (5-8 slides): pitch, proposal, QBR, all-hands |
| Stress tests | TP-25 – TP-26 | Minimal prompt (system invents everything), content dump (no structure) |

Each prompt is scored on 7 base axes (1-5 scale), plus 2 additional axes for multi-slide tests:

| Axis | What it measures | Scope |
|---|---|---|
| Content Fidelity | Does the output capture all user-specified content without fabrication? | All |
| Archetype Selection | Did the planner pick the right layout family for the content? | All |
| Visual Hierarchy | Is there a clear focal point and natural reading flow? | All |
| Density & Readability | Does the content fit without overflow, with appropriate whitespace? | All |
| Brand Consistency | Are all colors, fonts, and spacing from the design system tokens? | All |
| Editability | Can a non-designer edit the content and maintain layout quality? | All |
| Mechanical Defects | Does the geometry scan return zero blocking findings? | All |
| Cross-Slide Consistency | Do slides look like they belong to the same deck? | Multi-slide only |
| Narrative Flow | Does the deck tell a coherent story with clear progression? | Multi-slide only |

**Per-prompt pass**: average across all scored axes ≥ 3.5 AND no single axis ≤ 2.

**Benchmark pass** (V3 ships as default): ≥ 70% of test prompts pass AND V3 output is rated higher than V1 placeholder output on a majority of prompts by the user's own scoring.

The 7 base axes are aligned with the multimodal reviewer's 8-axis scoring (§4.8). `Content Fidelity` and `Archetype Selection` are human-only judgments. `Mechanical Defects` subsumes the deterministic scanner. The remaining 4 base axes map to reviewer axes. `Cross-Slide Consistency` and `Narrative Flow` are deck-level judgments that no single-slide reviewer can make.
...
### 10.2 Visual Hygiene Checks

26 binary pass/fail checks in `assets/benchmarks/v3_visual_hygiene_checks.xlsx` (generated by `scripts/generate_visual_hygiene_xlsx.py`). These test **mechanical correctness**, not content quality or taste.

| Category | Checks | Examples |
|---|---|---|
| Color (VH-01 – VH-05) | Palette fills, text colors, accent limit, contrast, invisible shapes | BLOCKING: off-palette fills; WARNING: low contrast |
| Typography (VH-06 – VH-09) | Font allowlist, type scale, bold usage, ALLCAPS | BLOCKING: wrong fonts; WARNING: excessive bold |
| Spatial (VH-10 – VH-15) | Off-canvas, overflow, safe area, overlaps, gutters, grid alignment | BLOCKING: off-canvas, overflow; WARNING: tight gutters |
| Content Rendering (VH-16 – VH-19) | Leaked markdown, placeholder text, image resolution, empty frames | BLOCKING: leaked markdown, placeholders |
| Cross-Slide Consistency (VH-20 – VH-23) | Title position, title style, kicker style, body font | WARNING: all (consistency, not correctness) |
| Structural (VH-24 – VH-26) | Empty slides, slide count, visual elements | BLOCKING: empty slides; WARNING: low visual ratio |

**Severity**: 12 BLOCKING (must pass for any deck to ship), 14 WARNING (reported, not gating).
...
5. On the 26 benchmark prompts (`assets/benchmarks/v3_test_prompts.xlsx`), ≥ 70% pass the evaluation rubric (§10.1), and V3 is rated higher than V1 on a majority of prompts.
